...
${envText}`;
    ...
    const domain = yamlObj.spec.hostname;
    ...
    for (
      ...
      const realValue = value.slice(4);
      for (const container of yamlObj.spec.containers) {
        if (container.env) {
          container.env.push({ name: key, value: realValue });
        }
      }
    }
...
const serviceIndex = [...pods.keys()];
const includeServices = await $.multiSelect({
  message: "Select services to include:",
  options: serviceIndex.map((name) => ({ text: name, selected: true })),
});
const unselectedServices = serviceIndex.filter((_name, i) =>
  !includeServices.includes(i)
);
unselectedServices.forEach((name) => {
  pods.delete(name);
  srvRecords.delete(name);
});
